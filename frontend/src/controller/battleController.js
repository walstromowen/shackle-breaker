...
export default class BattleController{
    ...
    createAbilityButtons(ally, resolveFn){
        ...
        //equipment abilities
        let combinedAbilities = this.model.getCombinedAbilities(ally, this.model.props.getBattle().canRetreat);
        ...
        for(let i = 0; i < combinedAbilities.length; i++){
            ...
            abilityButtons[i].addEventListener('mouseenter', (e)=>{
                e.preventDefault();
                ...
                abilityButtons[i].addEventListener('mouseleave', this.abilityMiniMenuExitEventHandler = (ev)=>{
                    abilityButtons[i].removeEventListener('mouseleave', this.abilityMiniMenuExitEventHandler);
        ...
        return new Promise((resolve)=>{
            document.addEventListener('click', this.skipEventHandler = ()=>{
                if(flag == true){
                    flag = false;
                    document.removeEventListener('click', this.skipEventHandler);
                    if(combatant.nextForm !== null ){
                        this.view.printToBattleConsole(combatant.playNextFormMessage());
        ...
        return new Promise((resolve)=>{
            document.addEventListener('click', this.skipEventHandler = ()=>{
                if(flag == true){
                    ...
                    let nextForm = this.model.formChange(combatant, combatant.createNextForm(this.model.props.getDifficulty()));
                ...
                if(resolveObject.switchCombatant){
                    ...
                    if(resolveObject.evade){
                        ...
                        if(resolveObject.retreat == false){
                            if(resolveObject.polymorph){
                                for(let i = 0; i < this.currentAttacker.abilityTargets.length; i++){
                                    this.model.formChange(this.currentAttacker.abilityTargets[i], this.currentAttacker.nextAbility.polymorph);
                                    this.currentAttacker = this.currentAttacker.nextAbility.polymorph;
                                }
                            }
    ...
    checkEndBattle(resolveFunction){
        ...
        if(remainingHostiles.length == 0){
            ...
            let partyPosition = this.model.props.getCurrentPartyPosition();
